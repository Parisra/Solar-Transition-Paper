...
def add_power_capacities_installed_before_baseyear(n, grouping_years, costs, baseyear):
    ...
    ##df_agg["lifetime"] = df_agg.DateOut - df_agg["grouping_year"] + 1   ##changed
    ...
    for grouping_year, generator in df.index:
      if not   baseyear<grouping_year: ## changed (otherwise adds 2025 to 2020 networks)
        ...
        if generator in ["solar", "onwind", "offwind"]:
            ...
            if "m" in snakemake.wildcards.clusters:
                for ind in new_capacity.index:
                    ...
                    inv_ind = [
                        i for i in inv_ind if (i + name_suffix) in n.generators.index.str ## changed
                        .replace(str(baseyear), str(grouping_year))
                    ]

                    p_max_pu = n.generators_t.p_max_pu[
                        [i + name_suffix.replace(str(grouping_year), str(baseyear)) for i in inv_ind]   ## changed
                    ]
                    p_max_pu.columns = [i + name_suffix for i in inv_ind]  ## changed

                    if baseyear==grouping_year :   ## changed
                        n.generators.loc[[i + name_suffix for i in inv_ind], 'p_nom_min'] = (new_capacity[ind]
                        / len(inv_ind))
...
if __name__ == "__main__":
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake(
            ...
            ll="v1.0",
            opts="",
            sector_opts="8760-T-H-B-I-A-dist1",#"1p7-4380H-T-H-B-I-A-dist1",
