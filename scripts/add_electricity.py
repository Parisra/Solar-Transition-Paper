...
if __name__ == "__main__":
    ...
    ## add costs for alternate solar configurations
    costs_ = pd.read_csv(snakemake.input.tech_costs,)
    ...
    tracking_costs= {'Investment': {'solar-W':1,'solar-E':1,
                       "solar-SW":1,"solar-SE":1,"solar-W-20":1,"solar-E-20":1,"solar-W-10":1, "solar-E-10":1,
                       "solar-W-60":1,"solar-E-60":1,"solar-W-90":1,"solar-E-90":1} , 
                      'FOM': {'solar-W':1,'solar-E':1,
                      "solar-SW":1,"solar-SE":1,"solar-W-20":1,"solar-E-20":1 ,"solar-W-10":1, "solar-E-10":1,
                      "solar-W-60":1,"solar-E-60":1,"solar-W-90":1,"solar-E-90":1}}     #Investment, O&M compred to no-tracking, direction has no effect
    extendables_solar_extra = [i for i in  params.electricity["extendable_carriers"]['Generator'] if 'solar-' in i]
    ...
    for solar_tech in extendables_solar_extra: #['solar-W','solar-E','solar-W-20','solar-E-20','solar-W-60','solar-E-60','solar-W-10','solar-E-10' ,'solar-hsat',  ##hsat is modified in prepare_sector
      ...
      if solar_tech not in costs_['technology'].values: 
        ...
        if 'inv' not in solar_tech:
                ...
                temp.loc[:,['technology']]=solar_tech
                costs_ = pd.concat([costs_,temp])
    for wind_tech in ['onwind-4MW','onwind-1.5MW']:
            if wind_tech not in costs_['technology'].values:     
                temp=pd.DataFrame({})
                temp=costs_.loc[costs_['technology']=='onwind']
                temp.loc[:,['technology']]=wind_tech
                costs_ = pd.concat([costs_,temp])        


    costs_.reset_index(drop=True,inplace=True)
    costs_.to_csv(snakemake.input.tech_costs, index=False)
