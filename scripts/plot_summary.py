...
def plot_carbon_budget_distribution(input_eurostat):
    ...
    emissions.loc[2019] = 3.414362 #2.971372
    emissions.loc[2020] = 3.092434 #2.691958
    emissions.loc[2021] = 3.290418 #2.869355
    emissions.loc[2022] = 3.213025

    if snakemake.config["foresight"] == "myopic":
        path_cb = "results/" + snakemake.params.RDIR + "/csvs/"
        co2_cap = pd.read_csv(path_cb + "carbon_budget_distribution.csv", index_col=0).squeeze()#[
        #    ["cb"]
        #]
        co2_cap *= e_1990
    else:
    ...
    ax1.plot(co2_cap, linewidth=3,)# label=col)
