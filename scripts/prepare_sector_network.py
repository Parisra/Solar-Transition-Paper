...
def build_carbon_budget(o, input_eurostat, fn, emissions_scope, report_year, input_co2):    
    ...
    e_0_ = 3.2
    print('start of path', t_0, type(t_0)) 
...
def remove_elec_base_techs(n):
    ...
    for c in n.iterate_components(snakemake.params.pypsa_eur):
        to_keep = snakemake.params.pypsa_eur[c.name] + [i for i in n.generators.carrier.unique() if 'solar' in i]   ##changed
...
def insert_electricity_distribution_grid(n, costs, distbat=True , distribution_efficiency= 1, rooftop_factor=1):  ## addeed last two params
    # TODO pop_layout?
    # TODO options?
    ...
    print('rooftop potential increased by factor of '+str(rooftop_factor))  
    ...
    if (distbat): # no home batteries are addedif distbat is False
...
def update_new_solar_costs (costs, config) :
    
    costs_ = pd.read_csv(costs)
    costs_['technology']= costs_['technology'].replace({"solar-utility single-axis tracking": "solar-hsat"})

    tracking_costs= {'Investment': {'solar-W':1,'solar-E':1,
                       "solar-SW":1,"solar-SE":1,"solar-W-20":1,"solar-E-20":1,"solar-W-10":1, "solar-E-10":1,"solar-W-60":1,"solar-E-60":1} , 
                      'FOM': {'solar-W':1,'solar-E':1,
                      "solar-SW":1,"solar-SE":1,"solar-W-20":1,"solar-E-20":1 ,"solar-W-10":1, "solar-E-10":1,"solar-W-60":1,"solar-E-60":1}}     #Investment, O&M compred to no-tracking, direction has no effect
    extendables_solar_extra = [i for i in  config["electricity"]["extendable_carriers"]['Generator'] if 'solar-' in i]

    for solar_tech in extendables_solar_extra: #['solar-W','solar-E','solar-W-20','solar-E-20','solar-W-60','solar-E-60','solar-W-10','solar-E-10' ,'solar-hsat',  ##hsat is modified in prepare_sector
                      #"solar-SW","solar-SE",] :
      if solar_tech not in costs_['technology'].values: 
        print('adding '+solar_tech+' costs')
        if 'inv' not in solar_tech:
                temp=pd.DataFrame({})
                temp=costs_.loc[costs_['technology']=='solar-utility']
                temp.loc[temp['parameter']=='investment','value']*=tracking_costs['Investment'][solar_tech]
                temp.loc[temp['parameter']=='FOM','value']*=tracking_costs['FOM'][solar_tech]
                temp.loc[:,['technology']]=solar_tech
                costs_ = pd.concat([costs_,temp])
        else:
                temp=pd.DataFrame({})
                base_solar_tech = 'solar-utility' if 'solar-hsat' not in solar_tech else 'solar-hsat'
                inverter_ratio = float(solar_tech.split('inv-')[-1])
                temp=costs_.loc[costs_['technology']==base_solar_tech]
                added_inv_costs = (0.01* temp.loc[temp['parameter']=='AC share of investment','value'].sum()* (1-inverter_ratio)+inverter_ratio) 
                temp.loc[temp['parameter']=='investment','value']*= added_inv_costs
                temp.loc[:,['technology']]=solar_tech
                costs_ = pd.concat([costs_,temp])

    costs_.reset_index(drop=True,inplace=True)
    costs_.to_csv(costs, index=False)

def add_new_solar_technologies(n, costs, rooftop_factor=1, pdelta=0.78,add_new_solar=False):
    
    if add_new_solar:
        
        print('adding new solar technologies')
        for solar_tech in [i for i in n.generators.carrier.unique() if 'solar' in i and not ('thermal' in i or 'rooftop' in i)]:              ##added
            index_tech = n.generators.index[n.generators.carrier == solar_tech]
            n.generators.loc[index_tech, "lifetime"] = costs.at["solar-utility", "lifetime"]
            if solar_tech!='solar' :  ##already updated in patch_elec
                print('updating costs for:', solar_tech)
                n.generators.loc[index_tech, "capital_cost"] = costs.at[solar_tech, "fixed"] 

            if 'inv' in solar_tech:  ## this is done manually since all the solar-inv techs are not in the config file to reduce the size  
                index_solar_inv = n.generators.index[n.generators.carrier == solar_tech]
                inv_ratio = float(solar_tech.split('-')[-1])
                n.generators.loc[index_solar_inv, "p_nom_max"]/= inv_ratio  ##is this really needed (besides accuracy) when we have the solar-potential constraint?
    
        ## insert delta configuration for utuility and rooftop
        ...
        n.madd(
           ...
           suffix="-delta",
            ...
            carrier="solar-delta",
            p_nom_extendable=True,
            p_nom_max=n.generators.loc[solar, "p_nom_max"]*1.2,     ## 20% higher land yeld
            marginal_cost=n.generators.loc[solar, "marginal_cost"],
        ...
        n.madd(
            ...
            carrier="solar-delta",
            p_nom_extendable=True,
            p_nom_max=n.generators.loc[solar, "p_nom_max"]*1.27,     ## 20% higher land yeld
            marginal_cost=n.generators.loc[solar, "marginal_cost"],
            capital_cost=n.generators.loc[solar, "capital_cost"]* pdelta,  ##0.66
            efficiency=n.generators.loc[solar, "efficiency"],
        ...
        n.madd(
            ...
            capital_cost=n.generators.loc[solar, "capital_cost"]* pdelta,  ##0.66
            efficiency=n.generators.loc[solar, "efficiency"],
            p_max_pu=df_east_west10/1.5,  ##assuming both energy and costs are lower
            lifetime=n.generators.loc[solar, "lifetime"],
        ) 
        ...
        AC_share_rooftop = (costs.at["solar-rooftop residential", "AC share of investment"]+costs.at["solar-rooftop commercial", "AC share of investment"])/2
        ...
        custom_settings= {'solar' : ['solar-utility', [1.7, 1.9,]],  #1.3, 1.5,
                      ...
                      'solar-SW':['solar-utility', [1.3, 1.5, 1.7, 1.9,]]
                      }  #1.3, 1.9
        
        for solar_tech in ['solar','solar-hsat']: #,'solar-E','solar-W','solar-SE','solar-SW'] : 
           solar_carrier = n.generators.index[n.generators.carrier == solar_tech]
           for inverter_ratio in custom_settings[solar_tech][1] :  ##1.3

                pmaxpu_inv = inverter_ratio * n.generators_t.p_max_pu[solar_carrier]
                pmaxpu_inv[pmaxpu_inv > (1)] = 1   ##DC/AC ratio
                capital_cost_inv = (costs.at[custom_settings[solar_tech][0], "fixed"]* (0.01*costs.at[custom_settings[solar_tech][0], "AC share of investment"]*
                    (1-inverter_ratio)+inverter_ratio) )  ## dc_part*r + ac_part = costs*(1-AC_share)*r + costs*AC_share = costs*(AC_share*(1-r)+r)

                n.madd(
                  "Generator",
                  solar_carrier,
                  suffix="-inv-"+str(inverter_ratio),
                  bus=n.generators.loc[solar_carrier, "bus"],
                  carrier=solar_tech+' inv',  #'solar-inv
                  p_nom_extendable=True,
                  p_nom_max=n.generators.loc[solar_carrier, "p_nom_max"]/inverter_ratio,  ##land_usage is by DC_capacity
                  marginal_cost=n.generators.loc[solar_carrier, "marginal_cost"],
                  capital_cost=capital_cost_inv,  ## 
                  efficiency=n.generators.loc[solar_carrier, "efficiency"],
                  p_max_pu=pmaxpu_inv,
                  lifetime=n.generators.loc[solar_carrier, "lifetime"],
                ) 
        ...
        for inverter_ratio in [1.3,1.5,1.7,1.9] :   #1.3,1.5,1.7,1.9 #1.5, 1.7
            ...
            df_solar_inv= pd.DataFrame(data = (n.generators_t.p_max_pu[solar_inv]).values, 
                      index = (n.generators_t.p_max_pu[solar]).index,
                              columns=solar,)    
            AC_share_rooftop = (costs.at["solar-rooftop residential", "AC share of investment"]+costs.at["solar-rooftop commercial", "AC share of investment"])/2
            capital_cost_inv = (costs.at["solar-rooftop", "fixed"]* (0.01*AC_share_rooftop*(1-inverter_ratio)+inverter_ratio) )  
             ## dc_part*r + ac_part = costs*(1-AC_share)*r + costs*AC_share = costs*(AC_share*(1-r)+r)
            n.madd(
              "Generator",
              solar,
              suffix=" rooftop-inv-"+str(inverter_ratio),
              bus=n.generators.loc[solar, "bus"] + " low voltage",
              carrier="solar rooftop",
              p_nom_extendable=True,
              p_nom_max=potential/inverter_ratio,
              marginal_cost=n.generators.loc[solar, "marginal_cost"],
              capital_cost=capital_cost_inv,
              efficiency=n.generators.loc[solar, "efficiency"],
              p_max_pu=df_solar_inv, #pmaxpu_inv,
              lifetime=costs.at["solar-rooftop", "lifetime"],
            ) 
        
        ## add extra rooftop configurations facing east or west
        for configuration in [ 
           ...
           n.madd(
            "Generator",
            solar,
            suffix=" rooftop "+configuration[2],
            bus=n.generators.loc[solar, "bus"] + " low voltage",
            carrier="solar rooftop",
            p_nom_extendable=True,
            p_nom_max=potential,
           ...
           for inverter_ratio in [1.9]:          ##1.9 is probably too high for prosumers? but gets selected
                ...
                df_solar_inv= pd.DataFrame(data = (n.generators_t.p_max_pu[solar_inv]).values, 
                      index = (n.generators_t.p_max_pu[solar]).index, columns=solar,)   
                capital_cost_inv = (costs.at["solar-rooftop", "fixed"]* (0.01*AC_share_rooftop*
                 (1-inverter_ratio)+inverter_ratio) )  

                n.madd(
                 "Generator",
                 solar,
                 suffix=" rooftop "+configuration[2]+"-inv-"+str(inverter_ratio),
                 bus=n.generators.loc[solar, "bus"] + " low voltage",
                 carrier="solar rooftop",
                 p_nom_extendable=True,
                 p_nom_max=potential/inverter_ratio,
...
def enforce_autarky(n, only_crossborder, level):
    ...
    if 1==1:
        ...
        #for parameter in ["s_nom", "s_nom_min",]:
        ...
        #for parameter in ["p_nom", "p_nom_min",]:
...
if __name__ == "__main__":
    ...
    costs_=pd.read_csv(snakemake.input.costs)
    for solar_tech in [i for i in  snakemake.config["electricity"]["extendable_carriers"]['Generator'] if 'solar-' in i]: 
        if solar_tech not in costs_['technology'].values: 
            print('updating solar costs for year:', snakemake.input.costs )
            update_new_solar_costs(snakemake.input.costs , snakemake.config)  ##TODO: fix run logic so it doesn't loop over prepare becuase of updated costs
    ...
    for o in opts:
        ...
        print('co2 limit for ', investment_year, 'is:',  limit)
        break
    for o in opts:
        if "Co2L" not in o:
            continue
    ...
    if options["electricity_distribution_grid"]:
      ...
      if "nohomebattery" in opts:                                                            ##  added
        insert_electricity_distribution_grid(n, costs, distbat=False, distribution_efficiency=dist_link_eff,rooftop_factor=rooftop_factor)      
      else  :
    ...
    for o in opts:
      if 'newsolar' in o:  
           ...
           for o in opts:
                if o[:6] == 'pdelta': 
                    pdelta = float(o[6:])/100.0   

    add_new_solar_technologies(n, costs,rooftop_factor=rooftop_factor, pdelta=pdelta, add_new_solar=add_new_solar)
